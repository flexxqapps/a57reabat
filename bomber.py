...
def do_zip_update():
    ...
    if success:
        mesgdcrt.SuccessMessage("a57 was updated to the latest version")
        mesgdcrt.GeneralMessage(
            "Please run the script again to load the latest version")
    else:
        mesgdcrt.FailureMessage("Unable to update a57-dev.")
        mesgdcrt.WarningMessage(
            "Grab The Latest one From https:t.me@barisflexxq")
...
def get_phone_info():
    while True:
        ...
        if ((len(target) <= 8) or (len(target) >= 15)):
            mesgdcrt.WarningMessage(
